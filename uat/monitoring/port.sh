...
if [ -z "$state1" ]; then
  MSG="${MSG}BMS RASA CORE service is down which is running $PORT1 port.\n"  
else
  echo "Connection to $SERVER on port $PORT1 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state2" ]; then
  MSG="${MSG}BMS RASA Action service is down which is running $PORT2 port.\n"  
else
  echo "Connection to $SERVER on port $PORT2 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state3" ]; then
  MSG="${MSG}BMS RASA NLG service is down which is running $PORT3 port.\n"  
else
  echo "Connection to $SERVER on port $PORT3 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state4" ]; then
  MSG="${MSG}Botler RASA Core service is down which is running $PORT4 port.\n"  
else
  echo "Connection to $SERVER on port $PORT4 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state5" ]; then
  MSG="${MSG}Botler RASA Action service is down which is running $PORT5 port.\n"  
else
  echo "Connection to $SERVER on port $PORT5 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state6" ]; then
  MSG="${MSG}Botler RASA NLG service is down which is running $PORT6 port.\n"  
else
  echo "Connection to $SERVER on port $PORT6 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state7" ]; then
  MSG="${MSG}CUSTODIAN RASA Core service is down which is running $PORT7 port.\n"  
else
  echo "Connection to $SERVER on port $PORT7 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state8" ]; then
  MSG="${MSG}CUSTODIAN RASA Action service is down which is running $PORT8 port.\n"  
else
  echo "Connection to $SERVER on port $PORT8 was successful"   > /dev/null 2>&1 
fi

if [ -z "$state9" ]; then
  MSG="${MSG}CUSTODIAN RASA NLG service is down which is running $PORT9 port.\n"  
else
